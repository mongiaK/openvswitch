...
#define IDLTEST_INDEXED_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = idltest_indexed_table_first(TABLE); \
...
#define IDLTEST_INDEXED_FOR_EACH(ROW, IDL) \
        for ((ROW) = idltest_indexed_first(IDL); \
             (ROW); \
             (ROW) = idltest_indexed_next(ROW))
#define IDLTEST_INDEXED_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = idltest_indexed_first(IDL); \
             (ROW) ? ((NEXT) = idltest_indexed_next(ROW), 1) : 0; \
             (ROW) = (NEXT))
...
void idltest_indexed_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
...
#define IDLTEST_SIMPLE5_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = idltest_simple5_table_first(TABLE); \
...
#define IDLTEST_SIMPLE5_FOR_EACH(ROW, IDL) \
        for ((ROW) = idltest_simple5_first(IDL); \
             (ROW); \
             (ROW) = idltest_simple5_next(ROW))
#define IDLTEST_SIMPLE5_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = idltest_simple5_first(IDL); \
             (ROW) ? ((NEXT) = idltest_simple5_next(ROW), 1) : 0; \
             (ROW) = (NEXT))
...
void idltest_simple5_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
...
#define IDLTEST_SIMPLE6_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = idltest_simple6_table_first(TABLE); \
...
#define IDLTEST_SIMPLE6_FOR_EACH(ROW, IDL) \
        for ((ROW) = idltest_simple6_first(IDL); \
             (ROW); \
             (ROW) = idltest_simple6_next(ROW))
#define IDLTEST_SIMPLE6_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = idltest_simple6_first(IDL); \
             (ROW) ? ((NEXT) = idltest_simple6_next(ROW), 1) : 0; \
             (ROW) = (NEXT))
...
void idltest_simple6_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
