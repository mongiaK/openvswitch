...
idltest_indexed_set_i(const struct idltest_indexed *row, int64_t i)
{
    ...
    key.integer = i;
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &idltest_indexed_col_i, &datum);
}
...
{
    ovsdb_idl_set_condition(idl, &idltest_table_indexed, condition);
}
...
idltest_simple5_set_name(const struct idltest_simple5 *row, const char *name)
{
    ...
    key.string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &idltest_simple5_col_name, &datum);
}
...
{
    ovsdb_idl_set_condition(idl, &idltest_table_simple5, condition);
}
...
idltest_simple6_set_name(const struct idltest_simple6 *row, const char *name)
{
    ...
    key.string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &idltest_simple6_col_name, &datum);
}
...
{
    ovsdb_idl_set_condition(idl, &idltest_table_simple6, condition);
}
