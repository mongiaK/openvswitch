...
main(int argc, char *argv[])
{
    ...
    fatal_ignore_sigpipe();  // 忽略pipe信号
    process_init();  // 初始化进程信号，创建pipe管道，sigaction设置等
    ...
    save_config__(config_tmpfile, &remotes, &db_filenames, sync_from,
                  sync_exclude, is_backup); // 将配置信息保存到文件

    daemonize_start(false);

    /* Load the saved config. */
    load_config(config_tmpfile, &remotes, &db_filenames, &sync_from,
                &sync_exclude, &is_backup);  // 从新从配置文件里面读取配置
              ...
              const char *sync_exclude, bool is_backup)
{
    ...
    if (ftruncate(fileno(config_file), 0) == -1) { // 修改config file的文件大小为0
        VLOG_FATAL("failed to truncate temporary file (%s)",
                   ovs_strerror(errno));
    }

    obj = json_object_create();  // 创建json对象
