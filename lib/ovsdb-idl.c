                                  ...
                                  const struct ovsdb_idl_column *column)
{
    for (size_t i = 0; i < class->n_tables; i++) {
        const struct ovsdb_idl_table_class *tc = &class->tables[i];
        if (column >= tc->columns && column < &tc->columns[tc->n_columns]) { // column 的地址在最大跟最小区间范围内，说明就是当前这个table
            return tc;
        }
    }
                      ...
                      const struct ovsdb_idl_column *column)
{
    ...
    const struct ovsdb_idl_table *table = ovsdb_idl_table_from_column(db,
                                                                      column);
    return &table->modes[column - table->class_->columns]; // 通过地址偏移大小索引到对应table的modes
}
