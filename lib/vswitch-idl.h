...
#define OVSREC_CT_TIMEOUT_POLICY_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = ovsrec_ct_timeout_policy_table_first(TABLE); \
...
#define OVSREC_CT_TIMEOUT_POLICY_FOR_EACH(ROW, IDL) \
        for ((ROW) = ovsrec_ct_timeout_policy_first(IDL); \
             (ROW); \
             (ROW) = ovsrec_ct_timeout_policy_next(ROW))
#define OVSREC_CT_TIMEOUT_POLICY_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = ovsrec_ct_timeout_policy_first(IDL); \
             (ROW) ? ((NEXT) = ovsrec_ct_timeout_policy_next(ROW), 1) : 0; \
             (ROW) = (NEXT))
...
void ovsrec_ct_timeout_policy_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
...
#define OVSREC_CT_ZONE_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = ovsrec_ct_zone_table_first(TABLE); \
...
#define OVSREC_CT_ZONE_FOR_EACH(ROW, IDL) \
        for ((ROW) = ovsrec_ct_zone_first(IDL); \
             (ROW); \
             (ROW) = ovsrec_ct_zone_next(ROW))
#define OVSREC_CT_ZONE_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = ovsrec_ct_zone_first(IDL); \
             (ROW) ? ((NEXT) = ovsrec_ct_zone_next(ROW), 1) : 0; \
             (ROW) = (NEXT))
...
void ovsrec_ct_zone_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
...
#define OVSREC_DATAPATH_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = ovsrec_datapath_table_first(TABLE); \
...
#define OVSREC_DATAPATH_FOR_EACH(ROW, IDL) \
        for ((ROW) = ovsrec_datapath_first(IDL); \
             (ROW); \
             (ROW) = ovsrec_datapath_next(ROW))
#define OVSREC_DATAPATH_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = ovsrec_datapath_first(IDL); \
             (ROW) ? ((NEXT) = ovsrec_datapath_next(ROW), 1) : 0; \
             (ROW) = (NEXT))
...
void ovsrec_datapath_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
