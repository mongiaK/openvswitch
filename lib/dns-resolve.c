...
{
    ub_ctx__ = ub_ctx_create(); // 系统函数，初始化一个context，初始化默认值，但是未读取/etc/hosts 和/etc/resolve.conf的值
    if (ub_ctx__ == NULL) {
    ...
    if (!filename || !stat(filename, &s) || errno != ENOENT) {
        int retval = ub_ctx_resolvconf(ub_ctx__, filename); //  将resolve.conf文件内容读入到上下文ctx里面
        if (retval != 0) {
    ...
    int retval = ub_ctx_hosts(ub_ctx__, NULL); // 读取默认的hosts文件到ctx
    if (retval != 0) {
        VLOG_WARN_RL(&rl, "Failed to read etc/hosts: %s",
                     ub_strerror(retval));
    }

    ub_ctx_async(ub_ctx__, true);  //  设置异步处理域名解析，true：表示创建新的后台线程去解析域名
    hmap_init(&all_reqs__);   // 初始化所有请求
