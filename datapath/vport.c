...
int ovs_vport_init(void)
{
	...
	err = lisp_init_module(); // 初始化lisp模块,创建lisp网络命名空间，注册lisp设备到内核
	if (err)
		goto err_lisp;
	err = gre_init(); // 注册gre网络协议的处理函数
	if (err && err != -EEXIST) {
		goto err_gre;
	} else {
		if (err == -EEXIST) {
		...
		err = ipgre_init(); // 如果gre协议初始化失败，那就初始化基于ipv4的gre协议处理函数
		if (err && err != -EEXIST) 
			goto err_ipgre;
		compat_gre_loaded = true;
	}
	err = ip6gre_init();  // 基于ipv6的gre协议初始化
	if (err && err != -EEXIST) {
		goto err_ip6gre;
	} else {
		if (err == -EEXIST) {
	...
	err = ip6_tunnel_init();  // ipv6的隧道技术初始化
	if (err)
		goto err_ip6_tunnel;
	else
...
// 网vport_ops_list注册ops, 如果是已注册的type则退出，返回已存在
