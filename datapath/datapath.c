...
static int __net_init ovs_init_net(struct net *net)
{
	...
	INIT_WORK(&ovs_net->dp_notify_work, ovs_dp_notify_wq);  // 初始化ovs消息通知模块
	ovs_netns_frags_init(net); // 初始化ovs的网络命名空间的ipv4 模块 , 处理ipv4的包
	ovs_netns_frags6_init(net); // 初始化ovs的网络命名空间的ipv6 模块，处理ipv6的包
...
static int __init dp_init(void)
{
	...
	ovs_nsh_init(); // 注册nsh网络层协议的gso回调到linux内核
	err = action_fifos_init(); // action flow初始化，结构体申请内存,使用alloc_percpu
	if (err)
		goto error;

	err = ovs_internal_dev_rtnl_link_register(); // 内核注册ovs网络设备，初始化datapath内部的vport
	if (err)
		goto error_action_fifos_exit;

	err = ovs_flow_init(); // 初始化flow模块，主要申请了cache
	if (err)
		goto error_unreg_rtnl_link;

	err = ovs_vport_init(); // 创建gre，lisp，tunnel等网络命名空间
	if (err)
		goto error_flow_exit;

	err = register_pernet_device(&ovs_net_ops); // 注册ovs的网络命名空间
	f (err)
		goto error_vport_exit;

	err = compat_init(); // 兼容性初始化
	if (err)
		goto error_netns_exit;

	err = register_netdevice_notifier(&ovs_dp_device_notifier); //向内核的设备变化通知链注册回调函数，当设备注册，状态变化时会执行此处的回调函数
	if (err)
		goto error_compat_exit;

	err = ovs_netdev_init(); // 注册netdev的vport_ops到ops_list, 当上层vswitchd发送创建网络设备命令会调用这里设置的回调函数
	if (err)
		goto error_unreg_notifier;

	err = dp_register_genl(); // 初始化datapath模块的netlink family，与用户态通信使用
	if (err < 0)
