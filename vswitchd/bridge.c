...
bridge_init(const char *remote)
{
    ...
    idl = ovsdb_idl_create(remote, &ovsrec_idl_class, true, true); // 连接ovsdb-server
    idl_seqno = ovsdb_idl_get_seqno(idl);
    ovsdb_idl_set_lock(idl, "ovs_vswitchd"); // 发送请求到ovsdb-server 获取名为ovs_vswitchd锁
    ovsdb_idl_verify_write_only(idl); // 设置写校验

    ovsdb_idl_omit_alert(idl, &ovsrec_open_vswitch_col_cur_cfg); // 忽略cfg列的告警
    ovsdb_idl_omit_alert(idl, &ovsrec_open_vswitch_col_statistics); // 忽略statistics 列的告警
    ...
    lacp_init();  // 注册lacp相关的command
    bond_init();   // 注册bond相关的command
    cfm_init();
    bfd_init();
    ovs_numa_init(); // ovs numa 初始化，主要是内存访问的优化，详情可以去查询numa相关信息
    stp_init(); // 注册stp相关的command
    lldp_init();
    rstp_init();
    ifaces_changed = seq_create();  // 创建一个seq，id唯一
    last_ifaces_changed = seq_read(ifaces_changed); // 获取ifaces的id
    ifnotifier = if_notifier_create(if_change_cb, NULL); // 创建PF_ROUTE的socket，获取路由变化通知 
...
bridge_run(void)
{
    ...
    ovsrec_open_vswitch_init(&null_cfg); // 初始化结构体里面的map

    ovsdb_idl_run(idl); // 收ovsdb-server的消息并处理

    if_notifier_run(); // 读取AF_ROUTE的socket消息，并执行相应notify callback
