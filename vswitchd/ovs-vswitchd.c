...
main(int argc, char *argv[])
{
    ...
    set_program_name(argv[0]); // 设置程序名称，ovs-vswitchd
    ovsthread_id_init(); //  初始化thread id，

    dns_resolve_init(true); // 解析本地的dns解析，/etc/hosts /etc/resolve.conf
    ovs_cmdl_proctitle_init(argc, argv); // argv参数的内存重新申请
    service_start(&argc, &argv); // linux 为空函数
    remote = parse_options(argc, argv, &unixctl_path); // 解析启动参数, 返回ovsdb-server的地址
    fatal_ignore_sigpipe(); // 程序忽略pipe信号

    daemonize_start(true); // 启动守护进程

    if (want_mlockall) { // 锁内存操作，进程使用的内存永远不会进入交换分区
#ifdef HAVE_MLOCKALL
    ...
    retval = unixctl_server_create(unixctl_path, &unixctl); // 创建unix socket server 启动listen
    if (retval) {
        exit(EXIT_FAILURE);
    }
    ...
    bridge_init(remote); // ovsdb-server初始化，注册command，监听本地route的变化
    free(remote);
    ...
    while (!exiting) {
        memory_run();
        if (memory_should_report()) { // 上报ovs 内存使用情况
            struct simap usage;
        ...
        bridge_run();
        unixctl_server_run(unixctl);  // accept 本地的unix socket
        netdev_run();
